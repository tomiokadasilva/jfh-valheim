...
RUN set -eux; \
  ...
  \
  echo "[build] Instalando Armoire ${ARMOIRE_VERSION}"; \
  mkdir -p /tmp/armoire; \
  curl \
  --fail \
  --silent \
  ...
  --location \
  --retry 3 \
  --output /tmp/armoire.zip \
  "https://thunderstore.io/package/download/Advize/Armoire/${ARMOIRE_VERSION}/"; \
  unzip -q /tmp/armoire.zip -d /tmp/armoire; \
  find /tmp/armoire -type f -name '*.dll' -print; \
  test -f /tmp/armoire/Armoire.dll; \
  cp /tmp/armoire/Armoire.dll /opt/jfh-mods/; \
  test -f /opt/jfh-mods/Armoire.dll; \
  \
  echo "[build] Instalando FuelEternal ${FUEL_ETERNAL_VERSION}"; \
  mkdir -p /tmp/fueletternal; \
  curl \
  --fail \
  --silent \
  --show-error \
  --location \
  --retry 3 \
  --output /tmp/fueletternal.zip \
  "https://thunderstore.io/package/download/Marf/FuelEternal/${FUEL_ETERNAL_VERSION}/"; \
  unzip -q /tmp/fueletternal.zip -d /tmp/fueletternal; \
  find /tmp/fueletternal -type f -name '*.dll' -print; \
  test -f /tmp/fueletternal/FuelEternal.dll; \
